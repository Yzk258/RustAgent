...
- **预设选项栏**：输入框上方的胶囊工具栏可固定选择 MC 版本、加载器（fabric / forge / neoforge / quartz 分段按钮）与找包数量（5/8/10/15/20 或自定义，单次对话上限 20，自动记忆），随每条消息一并发给 agent，省去"你想要什么版本"的追问，少花一轮 API 调用。单包用户所选 mod 上限 100 个（前置依赖自动补全与报错修复补入不计入）—— 为考虑轻量化，敬请谅解。
