...
2. **用户口味数据库**：喜欢/不喜欢反馈持续累积为标签权重（`userdata/userdata.db`），推荐随使用越来越精准。
...
- **聊天区**：与 CLI 同一套 Agent 对话，支持**流式输出**——回复逐字打出（打字机效果，不再等整个响应）；工具调用实时可见，组包时的实时进展带 ▰▱ 进度条（如"⏳ ▰▰▰▱… 3/8 正在收集 mod sodium"，同一行原地刷新）；LLM 思考期间也有实时反馈（"思考中 (Ns)" / "模型思考中… (Ns)"），全程不再是黑盒等待。多轮工具调用时，每段回复与工具调用行保持清晰的上下顺序（文本 → 工具行 → 文本 → … → 最终回复），不会互相穿插。
...
- **会话管理**：每轮对话结束后**默认自动保存**到 `userdata/sessions/auto-*.json`（同一会话固定覆盖写同一文件，打断/出错也保留已有内容），会话卡片显示当前自动保存文件；侧栏可新会话/另存快照/加载，会话记录列表点击即导入并完整渲染历史对话（合法文件才可导入）；也可打开 sessions 目录手动放入会话文件导入。
- **侧栏面板**：会话管理（新会话/保存/加载）、token 用量统计（含每轮增量）、用户口味标签权重、可用工具列表、已生成整合包列表。
