...
| `/new`               | 开启新会话（清空当前上下文）                                 |
| `/save`              | 手动另存一份会话快照到`sessions/`（含对话、token 用量，JSON 格式）；每轮对话结束后也会**自动保存**，无需手动操作 |
| `/load`              | 加载最近一次保存的会话                                       |
...
- **会话管理**：每轮对话结束后**默认自动保存**到 `sessions/auto-*.json`（同一会话固定覆盖写同一文件，打断/出错也保留已有内容），会话卡片显示当前自动保存文件；侧栏可新会话/另存快照/加载，会话记录列表点击即导入并完整渲染历史对话（合法文件才可导入）；也可打开 sessions 目录手动放入会话文件导入。
...
| `userdata.db`        | 用户口味数据库（反馈记录、标签权重）     |
| `sessions/*.json`    | 会话存档（完整上下文 + token 用量；`auto-*` 为每轮自动保存，`session-*` 为手动另存快照） |
