...
- **聊天区**：与 CLI 同一套 Agent 对话，支持**流式输出**——回复逐字打出（打字机效果，不再等整个响应）；工具调用实时可见，组包时的实时进展带 ▰▱ 进度条（如"⏳ ▰▰▰▱… 3/8 正在收集 mod sodium"，同一行原地刷新）。LLM 思考期间思考行实时显示阶段提示与秒数（"思考中 · 核对版本与加载器… (7s)"，提示语每 3 秒轮换）；推理型模型（返回 reasoning_content 的，如 deepseek-reasoner）还会在思考行实时滚动显示真实推理片段（💭 前缀）。多轮工具调用时，每段回复与工具调用行保持清晰的上下顺序（文本 → 工具行 → 文本 → … → 最终回复），不会互相穿插。
